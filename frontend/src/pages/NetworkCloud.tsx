...
export default function NetworkCloud() {
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <PageHeader
        title="Network & Cloud"
        subtitle="Cloud account connections, NPM/NDM product status, and network integration signals"
      />
      ...
      {!selectedScanId ? <EmptyState message="Run a scan to see network and cloud data" /> : (isLoading || analyticsLoading) ? (
        ...
        <>
          ...
          {/* NPM status */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`card ${npmEnabled ? 'border-green-200 bg-green-50/30' : 'border-border'}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-xl">🌐</span>
                  <h2 className="text-base font-semibold text-ink">Network Performance Monitoring</h2>
                </div>
                <DDLink href={ddUrl.npm(base)} label={npmEnabled ? 'View NPM' : 'Set up'} />
              </div>
              <div className={`text-sm ${npmEnabled ? 'text-green-700' : 'text-ink-muted'}`}>
                {npmEnabled
            ...
            <div className={`card ${ndmEnabled ? 'border-green-200 bg-green-50/30' : 'border-border'}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-xl">🔌</span>
              ...
              <div className={`text-sm ${ndmEnabled ? 'text-green-700' : 'text-ink-muted'}`}>
                {ndmEnabled
