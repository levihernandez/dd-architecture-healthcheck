...
const DRIFT_TYPE_LABELS: Record<string, { label: string; color: string }> = {
  missing_in_org: { label: 'Missing in Org', color: 'bg-red-500/15 text-red-400' },
  value_inconsistency: { label: 'Value Inconsistency', color: 'bg-orange-500/15 text-orange-400' },
  partial_adoption: { label: 'Partial Adoption', color: 'bg-amber-500/15 text-amber-400' },
  naming_drift: { label: 'Naming Drift', color: 'bg-yellow-500/15 text-yellow-400' },
};
...
export default function TagGovernance() {
  ...
  // Governance findings reference orgs by name only (not id) — resolve back to the
  // id so every mention can link to that org, not just the per-org profile cards.
  const orgIdByName = new Map(orgSummaries.map((o) => [o.orgName, o.orgId]));
    ...
    <div className="max-w-5xl space-y-8">
      ...
      <div className="flex gap-6 items-center card">
        ...
        <div className="flex-1 grid grid-cols-4 gap-4 text-center">
          ...
          <div>
            <div className="text-2xl font-bold text-violet-400">{globalTagKeys.length}</div>
            <div className="text-xs text-ink-muted">Global Tag Keys</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-green-400">{commonTagKeys.length}</div>
            <div className="text-xs text-ink-muted">Common Across All</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-red-400">{inconsistencies.length + valueDrift.length}</div>
              ...
              return (
                <div key={org.orgId} className="card">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <OrgQuickLink orgId={org.orgId} orgName={org.orgName} className="font-semibold" />
                      <div className="text-xs text-ink-faint">{org.tagKeys.length} tag keys detected</div>
                    </div>
                    <div className="text-right">
                      <div className={`text-sm font-bold ${ustPresent.length === 4 ? 'text-green-400' : ustPresent.length >= 2 ? 'text-amber-400' : 'text-red-400'}`}>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex flex-wrap gap-1">
                      {ustPresent.map((k) => (
                        <span key={k} className="text-xs bg-green-500/10 border border-green-500/30 text-green-400 px-1.5 py-0.5 rounded font-mono">{k} ✓</span>
                      ))}
                      {ustMissing.map((k) => (
                        <span key={k} className="text-xs bg-red-500/10 border border-red-500/30 text-red-400 px-1.5 py-0.5 rounded font-mono">{k} ✗</span>
                      ))}
                    </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        {issue.orgsWithKey.length > 0 && (
                          <div>
                            <div className="text-xs font-semibold text-green-400 mb-1">Present in</div>
                            <div className="flex flex-wrap gap-1">
                              {issue.orgsWithKey.map((o) => (
                                <OrgQuickLink
                                  key={o} orgId={orgIdByName.get(o) ?? ''} orgName={o} variant="inherit"
                                  className="text-xs bg-green-500/10 text-green-400 hover:text-green-400 px-1.5 py-0.5 rounded border border-green-500/30"
                                />
                              ))}
                            </div>
                          </div>
                        ...
                        {issue.orgsMissing.length > 0 && (
                          <div>
                            <div className="text-xs font-semibold text-red-400 mb-1">Missing from</div>
                            <div className="flex flex-wrap gap-1">
                              {issue.orgsMissing.map((o) => (
                                <OrgQuickLink
                                  key={o} orgId={orgIdByName.get(o) ?? ''} orgName={o} variant="inherit"
                                  className="text-xs bg-red-500/10 text-red-400 hover:text-red-400 px-1.5 py-0.5 rounded border border-red-500/30"
                                />
                              ))}
                            </div>
                          </div>
                      ...
                      <div className="text-xs">
                        <span className="font-semibold text-blue-400 uppercase tracking-wide">Recommendation: </span>
            ...
            {valueDrift.map((drift, i) => (
              <div key={i} className="card border-l-4 border-amber-400">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs bg-amber-500/15 text-amber-400 px-2 py-0.5 rounded-full font-medium">Value Drift</span>
                      ...
                      {Object.entries(drift.valuesByOrg).map(([org, values]) => (
                        <tr key={org}>
                          <td className="py-1.5 pr-4 whitespace-nowrap">
                            <OrgQuickLink orgId={orgIdByName.get(org) ?? ''} orgName={org} variant="inherit" className="font-medium text-ink-muted hover:text-dd-purple" />
                          </td>
                          <td className="py-1.5">
                            <div className="flex flex-wrap gap-1">
                              {(values as string[]).map((v) => (
                                <code key={v} className="bg-orange-500/10 border border-orange-500/30 text-orange-400 px-1.5 py-0.5 rounded">{v}</code>
                              ))}
                            </div>
                          </td>
                        </tr>
      ...
      {commonTagKeys.length > 0 && (
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {commonTagKeys.map((k) => (
              <code key={k} className="text-sm bg-green-500/10 border border-green-500/30 text-green-400 px-2.5 py-1 rounded font-mono">{k}</code>
            ))}
          </div>
        </section>
      )}
